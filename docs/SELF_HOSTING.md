...
- First start builds both images from this checkout, then downloads the exercise images/GIFs
  (~140 MB) once into `app/img` and `app/gif`. No Node needed locally — Docker does the build.
- Open **http://localhost:8080** and create a profile with a passkey.
- There are no prebuilt images to pull. `docker compose up` without `--build` used to fetch
  upstream openGym's containers, which do not contain this fork's code; the `image:` keys were
  removed rather than left as a trap. See the comment in `docker-compose.yml`.
...
**The `--build` is not optional.** Compose reuses the images it built last time, so `git pull
&& docker compose up -d` will start your old code against your new config — the most confusing
possible outcome, because nothing errors.
...
| `docker compose pull` reports nothing to pull | Expected: this fork publishes no images and builds from source. Use `docker compose up -d --build`. |
| Code changes don't show up after `git pull` | You left off `--build`, so compose restarted the previously built image. Re-run `docker compose up -d --build`. |
